...
const TARGET_COMP_NAME = "JOOLA Kup";

async function setupRootCompetition() {
    // 1. Find User
    ...
    if (!cSnap.empty) {
        ...
        const newComp = await addDoc(collection(db, "competitions"), {
            ...
            slug: 'joola-kup', // Simple slug
            organizationId: organizationId,
            type: 'tournament', // Acts as a container
            status: 'active',
            participantsCount: 0,
    ...
    console.log(`\nTARGET_COMPETITION_ID = "${compId}"`);
    console.log("Please update your migration script with this ID.");
}

setupRootCompetition().catch(console.error);
