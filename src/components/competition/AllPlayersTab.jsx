  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-xl md:text-2xl font-bold text-white">Svi Igrači</h2>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80">
          <div className="bg-slate-900 border border-red-500/30 rounded-2xl p-6 max-w-md w-full">
            ...
            <p className="text-slate-400 text-sm mb-6">
              Sigurno želite obrisati <span className="font-bold text-white">{filteredPlayers.length}</span> filtriranih igrača? 
      ...
      {filteredPlayers.length === 0 ? (
        <div className="text-center py-20 bg-slate-900 rounded-xl border border-slate-800">
          <UserX className="w-12 h-12 mx-auto mb-3 text-slate-700" />
          ...
          {filteredPlayers.map(player => (
            ...
            >
              ...
              {player.rating && (
                <div className="mb-4 pb-4 border-b border-slate-800">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-500">Rejting</span>
                    <span className="text-sm font-bold text-blue-400">{player.rating}</span>
                  </div>
                </div>
              )}
              ...
              <button
                ...
                className="w-full bg-red-600/10 hover:bg-red-600/20 text-red-400 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2"
              >
                <Trash2 size={12} />
                Obriši Igrača
              </button>
